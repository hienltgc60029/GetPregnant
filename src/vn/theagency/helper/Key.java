package vn.theagency.helper;

public class Key {

	public static final int ausgleichen = 104;
	public static final int auflosen = 103;
	public static final int verbessern = 102;
	public static final int vorbereiten = 101;
	public static final int unterstutzen = 100;

	public static final int linearAusgleichen = 1004;
	public static final int linearAuflosen = 1003;
	public static final int linearVerbessern = 1002;
	public static final int linearVorbereiten = 1001;
	public static final int linearUnterstutzen = 1000;
	
	public static final int AUSWAHL = 200;
	
	public static final int AUDIOS_NAME = 300;
	public static final int AUDIOS_DECRIPTION = 301;
	public static final int LISTVIEW_LIBRARY = 302;
	
	public static final int btn_deine_musik = 400;
	public static final int btn_back = 401;
	
	public static final int HOME = 501;
	public static final int PLAY = 502;
	
	
	public static final String BGTEXT = "bgtext";
	
	public static final int PLAYAUDIO = 700;
	public static final int PAUSEAUDIO = 701;
	public static final int NEXTAUDIO = 702;
	public static final int PREVAUDIO = 703;
	
	public static final int SEEKBAR_LINE = 704;
	public static final int SEEKBAR_VOLUME = 704;
	public static final int VOLUME = 705;
	public static final int SHUFFER = 706;
	public static final int REPEAT = 707;
	public static final int HEADER = 708;
	
	public static final int NOHITAKI_MusikCheck = 800;
	public static final int NOHITAKI_PDFCheck = 801;
	public static final int NOHITAKI_Wecker = 802;
	public static final int NOHITAKI_ChangeBg = 803;
	

	
	
	
	
}
